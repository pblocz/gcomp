...
class IntersectionBezier(FastIntersectionBezierCore):
    ...
    def __call__(self, polyA, polyB, epsilon=_EPSILON, firstCall=True):
        ...
        if self.intersect_bbox_check(polyA, polyB):
            ...
            if n * (n - 1) * DA > eps:
                polyA, (divA, divB) = polyB, self.division(polyA)
            elif m * (m - 1) * DB > eps:
                polyA, (divA, divB) = polyA, self.division(polyB)
    ...
    def _plot_points(self, polyA, k=3):
        '''
        ...
        r = self._plot_points(divA, k - 1)
        return np.concatenate((r, self._plot_points(divB[::-1], k - 1)))
