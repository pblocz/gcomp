...
def signature_check(Y1, I1, Y2, I2, eps = 0.1, step=0.01, threashold=0.1, plot=False):
    '''
    Comprueba si la signatura de Y1 en el intervalo I1, Y2 en el intervalo I2 son iguales.
    
    kwargs
    ------
    - eps: epsilon con el que comprobar los ceros
    - step: el paso usado para calcular los puntos de los intervalos I1,I2
    - threashold: el umbral a partir del cual considerar las curvas iguales
    ...
    # un mismo resultado siempre (por ejemplo idénticamente 0), en lugar de un array
    # devuelve un solo entero... Hemos tenido que hacer esto para resolverlo
    def transel(el,r):
    ...
    # Calculamos las distancias y contamos el número de elementos que están cerca
    ...
    print "[%s] y [%s] son iguales? %s" % (Y1,Y2,float(trues)/length >= threashold)
    return float(trues)/length >= threashold
